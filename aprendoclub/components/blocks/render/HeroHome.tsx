...
import { resolveMedia, resolveMediaList } from '@/lib/blocks/media'
import type { HeroHomeBlock as HeroHomeBlockType } from '@/payload-types'

/**
 * Render de 'heroHome': hero de la home con video de fondo, badge social
 * proof, título compuesto (tituloPre/Accent/Post), subtítulo, CTAs, stack de
 * avatares + rating. Espeja components/hero-section.tsx (framer-motion,
 * video de fondo, dots pattern, gradient overlay, scroll indicator).
 */
export function HeroHome({ block }: { block: HeroHomeBlockType }) {
  const ref = useRef(null)
  const isInView = useInView(ref, { once: true })
  const reduceMotion = useReducedMotion()

  const video = resolveMedia(block.videoBackground)
  const avatares = resolveMediaList(block.avatares)
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center gap-6 lg:gap-8 container-padding section-spacing max-w-5xl">
        ...
        {/* Social Proof Bar */}
        {(avatares.length > 0 || block.ratingTexto) && (
          <motion.div
        ...
        {block.microcopy && (
          <p className="text-sm text-gray-400">{block.microcopy}</p>
        )}
      </div>
