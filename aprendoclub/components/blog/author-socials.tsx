...
type Social = { platform?: string | null; url?: string | null; id?: string | null };
...
/** Iconos de redes sociales del autor (editables desde Payload). */
export function AuthorSocials({ socials }: { socials?: Social[] | null }) {
    ...
    <div className="flex items-center justify-center gap-3">
      {items.map((s) => (
        <a
          ...
          aria-label={LABELS[s.platform as string] ?? (s.platform as string)}
          className="text-gray-400 transition-colors hover:text-white"
        >
          {ICONS[s.platform as string] ?? <Globe className="h-5 w-5" />}
        </a>
      ))}
    </div>
  );
